...
void main() {
    ...
    vec2 centerOffset = iCenterOffset.xy * iAnimationSize;
    vec2 coord = (fc - iResolution.xy * 0.5) * scale - centerOffset;
    vec2 mouseW = (iMouse.xy - iResolution.xy * 0.5) * scale - centerOffset;
    float m1 = 0.0;
    for (int i = 0; i < 50; i++) {
    ...
    useEffect(() => {
        ...
        const program = new Program(gl, {
            ...
            uniforms: {
                ...
                iBallCount: { value: Math.min(ballCount, MAX_BALLS) },
                iCursorBallSize: { value: cursorBallSize },
                iMetaBalls: { value: metaBallsUniform },
                iClumpFactor: { value: clumpFactor },
                enableTransparency: { value: enableTransparency },
                iCenterOffset: { value: new Vec3(0, centerOffsetY, 0) },
            },
        });
        ...
        function update(t: number) {
            ...
            if (pointerInside) {
                targetX = pointerX;
                targetY = pointerY + gl.canvas.height * centerOffsetY;
            } else {
                const cx = gl.canvas.width * 0.5;
                const cy = gl.canvas.height * (0.5 + centerOffsetY);
