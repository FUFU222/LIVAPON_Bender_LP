...
const dotVal = r[0] _ (r_yzx[0] + 33.33) + r[1] _ (r_yzx[1] + 33.33) + r[2] \* (r_yzx[2] + 33.33);
for (let i = 0; i < 3; i++) {
...
let p = [v[0] _ 0.1031, v[1] _ 0.103, v[2] _ 0.0973].map(fract);
const p_yxz = [p[1], p[0], p[2]];
const dotVal = p[0] _ (p_yxz[0] + 33.33) + p[1] _ (p_yxz[1] + 33.33) + p[2] _ (p_yxz[2] + 33.33);
for (let i = 0; i < 3; i++) {
...
vec2 coord = (fc - iResolution.xy _ 0.5) _ scale;
vec2 mouseW = (iMouse.xy - iResolution.xy _ 0.5) _ scale;
float m1 = 0.0;
for (int i = 0; i < 50; i++) {
...
cFinal = iColor _ alpha1 + iCursorColor _ alpha2;
}
